...
modules = ["client_error", "house", "outside", "user_rating", "mail", "avatar",
           ...
           "passport", "player", "shop", "mobile", "confirm",
           "craft", "profession", "inventory", "event","chatdecor","chatdecor" ,
           "clan", "clan_request", "clan_member","clan_activity", "clan_news","clan_location"]
...
class Server():
    ...
    def _background(self):
        while True:
            self.redis.delete(f"online_tr")
            for uid in self.inv.copy():
                inv = self.inv[uid]
                self.redis.sadd(f"online_tr", uid)
