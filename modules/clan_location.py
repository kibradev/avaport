...
class ClanLocation(Location):
    ...
    def get_room(self, msg, client):
        ...
        client.send(["c.gr", {"rid": client.room}])
    ...
    def room(self, msg, client):
        ...
        if subcommand == "info":
            ...
            try:
                ...
                client.send(["cp.ms.rsm", {"txt": "Произошла ошибка, "
                                                        "перезайдите в игру"}])
                return
            ...
            cid = msg[0].split("_")[1]
            tid = self.server.redis.get(f"clans:{cid}:room")
            room = {"f": [], "w": 13, "id": "hall", "lev": 1, "l": 13,
                    "tid": tid, "nm": ""}
            evn = None
            client.send(["c.r.info", {"rmmb": rmmb, "rm": room,
                                            "evn": evn}])
        else:
